...
function doSub(){
	$.ajax({
		cache: true,
		type: "POST",
		url:"${basePath}/admin/config/ifm/edit",
		data:$('#configEdit').serialize(),
		async: false,
...
<body>
	<form id="configEdit" action="" method="post">
		...
		<div title="参数信息" style="padding:30px;height:400px;" fit=true>
			<table width="90%">
				<tr>
					...
					<td><input name="name" value="${config.name }" data-options="required:true"/></td>
				</tr>
				<tr>
					<td height="45px" width="150px">参数值:</td>
					<td><input name="value" value="${config.value }" data-options="required:true"/></td>
				</tr>
				<tr>
					<td>参数描述:</td>
					<td><textarea name="des" rows="5" style="width:280px" data-options="required:true">${config.des }</textarea></td>
				</tr>
				<tr>
