...
function doSub(){
	...
	$.ajax({
		cache: true,
		type: "POST",
		url:"${basePath}/admin/role/ifm/edit",
		...
		<div title="角色信息" style="padding:30px;height:400px;" fit=true>
			<table width="90%">
				<tr>
					...
					<td><input name="name" id="roleName" value="${role.name }" data-options="required:true"/></td>
				</tr>
				<tr>
					<td>角色描述:</td>
					<td><textarea name="des" id="roleDes" rows="5" style="width:280px" data-options="required:true">${role.des }</textarea></td>
				</tr>
				<tr>
					<td height="45px">状态:</td>
					<td>
						<input type="radio" value="1" <c:if test='${role.status eq 1}'>checked="checked"</c:if> name="status" data-options="required:true"/>启用
						<input type="radio" value="2" <c:if test='${role.status eq 2}'>checked="checked"</c:if> name="status" data-options="required:true"/>停用
					</td>
				</tr>
				<tr>
					<td colspan="2">
					...
					<table cellpadding="0" cellspacing="0">  
						...
						<c:forEach items="${listRights }" var="right">
							...
							<c:if test="${right.type eq rightType.id }">
								<c:forEach items="${role.listRights }" var="roleRight">
									<c:if test="${right.id eq roleRight }">
