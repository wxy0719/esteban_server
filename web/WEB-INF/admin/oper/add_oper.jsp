...
function doSub(){
	...
	$.ajax({
		cache: true,
		type: "POST",
		url:"${basePath}/admin/oper/ifm/add",
		...
		<div title="角色信息" style="padding:30px;height:400px;" fit=true>
			<table width="90%">
				...
				<tr>
					...
					<td>
						<input type="radio" value="1" name="status" checked="checked" data-options="required:true"/>启用
		...
		<table> 
			...
			<c:forEach items="${listRole }" var="role" begin="0" varStatus="status"> 
				<c:if test="${status.index%5==0&&status.index!=0}">
					...
					<td><input name="rights" type="radio" value="${role.id }"/>${role.name }</td>
				</c:if> 
				<c:if test="${status.index%5!=0||status.index==0}">
					<td><input name="rights" type="radio" value="${role.id }"/>${role.name }</td>
				</c:if>				
